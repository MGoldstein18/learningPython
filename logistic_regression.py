...
passengers.Age.fillna(value=round(np.mean(passengers.Age), 0), inplace=True)
...
prediction_probabilities = model.predict_proba(sample_passengers)
predictions = model.predict(sample_passengers)

print('Probabilities:')
print(prediction_probabilities)
print('Predictions:')
print(np.where(predictions == 1, 'Survived', 'Perished'))
